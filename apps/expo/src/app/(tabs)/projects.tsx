...
const AreaCard = ({ area }: { area: Area }) => {
  return (
    <View className="bg-slate-800 mb-1 flex h-14 flex-row items-center gap-4 rounded-lg p-2">
...
const NewAreaCard = ({
  ...
  return (
    <View className="bg-slate-800 mt-2 flex flex-row items-center gap-4 rounded-lg p-2">
      <TextInput
        value={newArea}
        onChangeText={(t) => setNewArea(t)}
        onSubmitEditing={handleSubmit}
        placeholder="Add a new project..."
        className="placeholder:text-gray-400 w-full p-2 text-xl text-white"
      />
    </View>
  );
};
