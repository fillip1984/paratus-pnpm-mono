...
import type { RoutineWithScheduledDays } from "~/stores/routineStore";
import { createActivitiesFromRoutine } from "~/stores/activityStore";
import { createRoutine, findRoutines } from "~/stores/routineStore";
import { h_mm_ampm, HH_mm_aka24hr } from "~/utils/date";
...
export default function PlannerScreen() {
  ...
  const importSampleRoutines = async () => {
    ...
    for (const routine of sampleRoutines) {
      ...
      if (!newRoutine.routineResult[0]) {
        throw Error("Failed to create a new routine");
      }
    ...
    <SafeAreaView className="bg-black">
      <View className="flex h-screen bg-black px-2">
        <Link href="/(routines)/new" asChild>
          <Text className="text-stone-100 mb-2 ml-auto text-2xl font-extrabold">
            Add
          </Text>
        </Link>
        {routines.length === 0 && (
...
const RoutineCard = ({ routine }: { routine: RoutineWithScheduledDays }) => {
  return (
    <Link href={`/(routines)/${routine.id}`} asChild>
      <Pressable className="bg-stone-400 flex w-full rounded p-2">
