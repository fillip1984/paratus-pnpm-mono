...
import { findTodo, updateTodo } from "~/stores/todoStore";
...
export default function TodoDetails() {
  ...
  const startStopTimer = async () => {
    ...
    if (notificationId !== undefined && id) {
      void unscheduleNotificationsForTodo(id, notificationId);
    }
  };
  ...
  const reset = () => {
    ...
    if (!id) {
      throw Error("Notification id is not found");
    }
    void unscheduleNotificationsForTodo(id);
    ...
    <SafeAreaView className="bg-stone-800">
      <View className="bg-stone-900 flex h-full gap-8 px-2">
        ...
        <View className="flex flex-row gap-2">
          <Pressable
            onPress={reset}
            className="bg-orange-500 flex items-center justify-center rounded px-4 py-2">
            <Text className="text-4xl text-white">Reset</Text>
          </Pressable>
          <Pressable
            onPress={startStopTimer}
            className="bg-blue-500 flex flex-1 items-center justify-center rounded px-4 py-2">
