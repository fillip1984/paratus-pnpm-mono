...
export default function RoutineDetails() {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Yes",
          style: "destructive",
          onPress: void handleDeleteRoutine,
        },
      ],
    );
  };
   ...
   *
   */
  const parseDateOrToday = (str: string | undefined) => {
  ...
  return (
    <SafeAreaView className="bg-stone-800">
      <View className="bg-stone-900 flex h-full gap-8 px-2">
        <TopActionsBar saveRoutine={handleSaveRoutine} />
        <ScrollView>
          <View className="flex flex-1 gap-8">
            <View className="bg-stone-800 flex gap-2 rounded-lg p-1">
              <TextInput
                className="placeholder:text-stone-500 px-3 py-2 text-xl text-white"
                value={name}
                onChangeText={(e) => setName(e)}
                placeholder="Name of routine"
              />
              <TextInput
                className="border-t-stone-700 placeholder:text-stone-500 min-h-[100px] border-t px-3 py-2 text-xl text-white"
                multiline
                numberOfLines={3}
                value={description}
        ...
        {id && (
          <View className="mb-2 flex-row gap-2">
            <Pressable
              ...
              className="border-stone-300 flex flex-1 items-center rounded border p-2">
              <Text className="text-stone-300 text-2xl">Rebuild</Text>
            </Pressable>
            <Pressable
              onPress={handleDeleteConfirmation}
              className="border-red-500 flex items-center rounded border p-2">
              <Text className="text-red-500 text-2xl">Delete</Text>
            </Pressable>
          </View>
        )}
      </View>
